...
/// 检测路径中是否存在 `tft<数字>_` 形式的赛季资源前缀（如 tft9_、tft18_）。
/// 轻量扫描实现，避免为此引入完整正则依赖。
fn has_tft_season_prefix(lower: &str) -> bool {
    let bytes = lower.as_bytes();
    let mut i = 0;
    while i + 4 < bytes.len() {
        if bytes[i] == b't' && bytes[i + 1] == b'f' && bytes[i + 2] == b't' {
            let mut j = i + 3;
            let digit_start = j;
            while j < bytes.len() && bytes[j].is_ascii_digit() {
                j += 1;
            }
            if j > digit_start && j < bytes.len() && bytes[j] == b'_' {
                return true;
            }
        }
        i += 1;
...
fn get_tft_local_cache_path(path: &str) -> Option<PathBuf> {
    ...
    let is_tft = lower_path.contains("/tft/")
        ...
        || has_tft_season_prefix(&lower_path)
...
mod tests {
    ...
    #[test]
    fn tft_season_prefix_matches_current_and_future() {
        assert!(has_tft_season_prefix("assets/tft9_champion.tft9_1.png"));
        assert!(has_tft_season_prefix("assets/tft13_something.png"));
        assert!(has_tft_season_prefix("assets/tft17_item.png"));
        assert!(has_tft_season_prefix("assets/tft18_newseason.png"));
        assert!(has_tft_season_prefix("assets/tft20_future.png"));
    }

    #[test]
    fn tft_season_prefix_rejects_non_tft() {
        assert!(!has_tft_season_prefix("assets/champion-icons/ahri.png"));
        assert!(!has_tft_season_prefix("assets/sounds/tft_theme.ogg"));
        // "tft" 后没有数字下划线不算赛季前缀
        assert!(!has_tft_season_prefix("assets/tft_champion_icons/x.png"));
        // "tft" 后是数字但没有下划线也不算
        assert!(!has_tft_season_prefix("assets/something-tft123.png"));
        assert!(!has_tft_season_prefix("assets/empty"));
    }

    #[test]
    fn tft_cache_path_detects_season_assets() {
