...
//! 统一走 `with_db` / `spawn_blocking`，避免在 async worker 上同步锁库。

use tauri::{AppHandle, Manager};

use super::payload::UploadPayload;
use crate::saved_players::with_db;
...
pub async fn load_pending_uploads(app_handle: &AppHandle) -> Vec<PendingUploadItem> {
...
pub async fn add_or_update_pending_upload(
    ...
    let state = app_handle.state::<crate::AppState>();
    let result = with_db(state.inner(), move |conn| {
    ...
    if let Err(e) = result {
        log::error!("写入 pending_uploads {} 失败: {}", game_id, e);
    }
}

/// 上传成功后，清除该条 SQLite 数据库记录
pub async fn remove_pending_upload(app_handle: &AppHandle, game_id: u64) {
    let state = app_handle.state::<crate::AppState>();
    let result = with_db(state.inner(), move |conn| {
        conn.execute(
        ...
        .map(|_| ())
        .map_err(|e| format!("删除 pending_uploads 记录 {} 失败: {}", game_id, e))
    })
    .await;
